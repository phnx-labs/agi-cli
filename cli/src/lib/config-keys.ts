...
export function parseConfigKey(key: string): ParsedConfigKey {
  ...
  const deviceMatch = raw.match(
    /^devices\.(.+)\.(role|max-agents|scheduler|daemon|watchdog|tmux|notes|formFactor|browser\.remote-control|browser\.task-idle-minutes|browser\.profile|browser\.viewer)$/,
  );
  if (deviceMatch) {
    return {
      scope: 'device',
