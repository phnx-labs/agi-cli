...
describe('account slots (PHNX-3940)', () => {
  ...
  it('falls through to the recorded-version tier when the launch id names no known slot', () => {
    // An unresolved sidecar id (never populated) or a since-removed slot must not
    // throw or silently darken a row that tier 2 could otherwise resolve truthfully.
    const index = buildClaudeAccountIndex();
  ...
  it('does NOT let a launch id override tier 1 path evidence for a session that '
    + 'genuinely lives in a different, identifiable home', () => {
    // Physical location is still the strongest evidence: a launch-recorded id is only
    // an escape hatch for rows sitting OUTSIDE every identifiable home (tier 2's
    // territory), never a way to relabel a transcript that demonstrably sits inside
    // some other home.
    const index = buildClaudeAccountIndex();
