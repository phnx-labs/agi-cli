...
/** A list-item or lead-in line that belongs WITH a trailing ask block. */
function isAskLeadIn(line: string): boolean {
  const t = line.trim();
...
export interface ProseQuestion {
  /** The trailing ask, collapsed to one logical line (multiple asks kept, joined). */
  text: string;
 ...
 *     in `text`.
 *   - `context` preserves newlines/Markdown; `text` is one collapsed line.
 */
export function extractProseQuestion(raw: string): ProseQuestion | undefined {
  ...
  for (const line of lines) {
    const t = line.trim();
    if (/^(```|~~~)/.test(t)) { inFence = !inFence; eligible.push(false); continue; }
    if (inFence) { eligible.push(false); continue; }
  ...
  for (let i = qEnd - 1; i >= 0; i--) {
    ...
    if (lineIsQuestion(lines[i]) || isAskLeadIn(lines[i])) blockStart = i;
    else break;
  }
  ...
  // Same-line case: peel leading non-question sentences into context.
  let inlineContext = '';
  if (!block.includes('\n')) {
  ...
  const context = [contextAbove, inlineContext].filter(Boolean).join('\n\n').trim() || undefined;
  return { text: oneLine(block), context };
}
