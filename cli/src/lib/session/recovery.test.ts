...
describe('resolveSessionRecoveryFromCandidates', () => {
  it('native-resumes only the healthy origin version', () => {
    ...
    try {
      ...
      const result = resolveSessionRecoveryFromCandidates(
        source,
        [candidate('2.1.187', { slotDir: home }), candidate('2.1.218')],
        () => true,
        inspection,
      );
  ...
  it('native-resumes the origin home on a rotated provider account when the origin login is limited', () => {
    ...
    try {
      ...
      const source = session({ filePath, cwd, accountId: 'origin-account' });
      const inspection = inspectNativeResumeSession(source, home);
      const result = resolveSessionRecoveryFromCandidates(
        source,
        [
  ...
  it('uses /continue on a healthy same-harness version when the origin is signed out', () => {
    const result = resolveSessionRecoveryFromCandidates(
      ...
      [candidate('2.1.187', { signedIn: false, nativeAccountId: 'origin-account' }), candidate('2.1.218')],
      () => true,
    );
    expect(result).toMatchObject({ mode: 'continue', agent: 'claude', version: '2.1.218' });
  ...
  it('keeps a healthy origin home for /continue when the harness has no native resume form', () => {
    const result = resolveSessionRecoveryFromCandidates(
      session({ accountId: 'origin-account' }),
      [candidate('2.1.187', { nativeAccountId: 'origin-account' }), candidate('2.1.218')],
      () => false,
    );
...
describe('sessionMatchesAccount', () => {
  it('matches on the persisted sidecar accountId regardless of path', () => {
    ...
    expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-1', agent: 'claude' })).toBe(true);
    expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-2', agent: 'claude' })).toBe(false);
  });

  it('never matches across a different agent, even with the same accountId', () => {
    const s = { agent: 'claude' as const, filePath: '/anywhere/session.jsonl', accountId: 'acct-1' };
    expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-1', agent: 'codex' })).toBe(false);
  });
  ...
  it('falls back to canonical transcript ownership when there is no sidecar accountId', () => {
    ...
    try {
      const home = path.join(root, 'home');
      fs.mkdirSync(home, { recursive: true });
      const filePath = path.join(home, '.claude', 'projects', 'transcript.jsonl');
      fs.mkdirSync(path.dirname(filePath), { recursive: true });
      fs.writeFileSync(filePath, '{}\n');
      const s = { agent: 'claude' as const, filePath };
      expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-1', agent: 'claude' }, home)).toBe(true);
      expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-1', agent: 'claude' }, path.join(root, 'other-home'))).toBe(false);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it('stays unknown (false) with no accountId and no home to prove ownership', () => {
    ...
    expect(sessionMatchesAccount(s, { kind: 'native', id: 'acct-1', agent: 'claude' })).toBe(false);
  });
});
...
describe('account context survives binary changes', () => {
  it('uses the current binary with the original context label and rejects retired files inside that home', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-recovery-current-'));
    try {
      const fixture = nativeHomeFixture(root, session().id);
      const source = session({ filePath: fixture.filePath, cwd: fixture.cwd, version: '2.1.187' });
      const oldHome = candidate('2.1.187', { slotDir: fixture.home });
      const result = resolveSessionRecoveryFromCandidates(source, [oldHome], () => true, undefined, { executableVersion: 'main' });
      expect(result).toMatchObject({ mode: 'native', version: 'main', execHome: fixture.home, configVersion: '2.1.187' });
      expect(result.candidate).toBe(oldHome);
      const retired = path.join(fixture.home, '.claude.superseded', 'projects', 'old.jsonl');
      fs.mkdirSync(path.dirname(retired), { recursive: true });
      fs.copyFileSync(fixture.filePath, retired);
      expect(inspectNativeResumeSession({ ...source, filePath: retired }, fixture.home).available).toBe(false);
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  });

  it('does not replace a missing recorded account with another account in the same binary home', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-recovery-missing-account-'));
    try {
      const fixture = nativeHomeFixture(root, session().id);
      const source = session({ filePath: fixture.filePath, cwd: fixture.cwd, accountId: 'removed-account' });
      const result = resolveSessionRecoveryFromCandidates(source, [candidate('2.1.187', { slotDir: fixture.home, nativeAccountId: 'replacement-account' })], () => true);
      expect(result.mode).toBe('continue');
      expect(result.reason).toContain('recorded account is not available');
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  });
});

describe('verified transcript requirement', () => {
  it('rejects a live or mirrored metadata stub without conversation content', async () => {
    expect(() => assertRecoverableTranscript(session({ filePath: '' }))).toThrow('No agent was started');
    expect(() => assertRecoverableTranscript(session({ filePath: '', mirrorSyncedAt: Date.now(), mirrorSource: 'offline-worker' }))).toThrow('No agent was started');
    const { upsertMirrorSession, getSessionById } = await import('./db.js');
    const id = '22222222-2222-3333-4444-555555555556';
    upsertMirrorSession({ id, shortId: id.slice(0, 8), agent: 'claude', machine: 'offline-worker', timestamp: '2026-09-12T00:00:00Z', firstUser: 'Only a preview snippet was synced.' }, 'offline-worker', Date.now());
    expect(() => assertRecoverableTranscript(getSessionById(id)!)).toThrow('No agent was started');
  });

  it('accepts real archived content and rejects empty transcript files', async () => {
    const { upsertSession } = await import('./db.js');
    const archived = session({ id: '22222222-2222-3333-4444-555555555555', filePath: '' });
    upsertSession(archived, 'A prior user request that can be replayed.');
    expect(() => assertRecoverableTranscript(archived)).not.toThrow();
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-recovery-empty-'));
    try {
