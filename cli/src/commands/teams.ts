...
export function registerTeamsCommands(program: Command): void {
  ...
  addHostOption(teams.command('add <team> <teammate> <task>'))
    ...
    .action(async (team: string, teammate: string, task: string, opts: {
      ...
      const { agent, version: requestedVersion, profileName, account } = parseTeammate(teammate);
      const version = hostName || cloudProviderId
        ? requestedVersion
        : resolveVersionAlias(agent, requestedVersion);
      ...
      if (version && !hostName && !cloudProviderId && !isVersionInstalled(agent, version)) {
        dieFriction(
          'teams',
