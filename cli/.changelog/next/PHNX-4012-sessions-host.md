- **`agents sessions … --host <target>` forwards to the standalone `sessions` CLI (PHNX-4012).**
  ...
  `sessions` is ≥ 0.3.0, mirroring how the 0.2.0 filter/sort flags are version-gated. A box with
  `sessions` below the 0.3.0 host floor — or none installed — keeps the query on the in-repo engine
