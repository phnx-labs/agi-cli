- **Fleet probes follow the configured `platform`, not the discovered one.** A box Tailscale
  ...
  worked. Every remote-shell decision now resolves the operator profile first, the same profile
  the dial already used. Source: `cli/src/lib/devices/health.ts` (`buildProbeInvocation`),
