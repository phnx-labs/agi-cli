...
- **A full sync no longer deletes skill directories it never wrote.** The skills orphan sweep removed every non-dot directory under a harness's `skills/` that was not in the current sync set. That included Claude Code's own `skills/synced` bucket, so running the full sync that clears the flattened plugin-skill copies left by 1.22.117 (`animator` beside `create:animator`) would also have deleted that bucket. A directory is now swept only if the previous full sync's manifest lists it in `writtenTargets`, the same bound the commands sweep uses. With no manifest, nothing is swept. Source: `cli/src/lib/installations/versions.ts`, `cli/src/lib/installations/versions.test.ts`.
