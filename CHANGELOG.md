...
- **A PreToolUse hook whose script is missing now denies the tool call instead of allowing it.** Generated hook shims embed one source path inside a version home; when that file was gone (a pruned version, sync rewriting `hooks/`) the shim exited 127, which Claude Code, Codex and Droid read as "allow". On one laptop the primary-checkout guard was silently disabled for about 17 hours this way. A `PreToolUse` shim now exits 2 with the repair named (`agents hooks sync`) and logs `cache: missing-source`, after the `matches:` gate and only for a PreToolUse firing; other events and skipped fires stay fail-open. `agents prune <harness>@<version>` also re-points the shims at the surviving home immediately instead of waiting for the daemon.

- **Claude no longer gets every plugin skill twice.** Sync flattened each plugin's `skills/` into the top-level `skills/` of every harness, but Claude Code already loads them from the plugin registration, namespaced (`/sessions:continue`), so the picker listed `/continue` and `/sessions:continue` side by side for 47 skills. Claude is flagged `nativePluginSkills`: sync stops flattening there, and the leftover copies are swept by the next full `agents sync claude` (or `agents prune cleanup skills`). Harnesses that only see plugin skills through the flat copy (Codex) are unchanged.

- **The retired Gemini agent is removed entirely.** Google retired the Gemini CLI (June 2026; Antigravity is the successor), but the leftover `gemini` agent spec kept re-creating dead `GEMINI.md → AGENTS.md` alias symlinks in every synced project and home dir (via `compileRulesForProject`), on top of other dead cruft. Gemini is now deleted from the registry (`AGENT_IDS` in `types.ts` and the `AGENTS` spec) and from every `AgentId`/`SessionAgentId`/`AgentType` union, `Record<AgentId>` map, and dispatch branch across the CLI (~54 files, ~1130 lines removed). `agents sync` no longer creates `GEMINI.md`; the symlink loop also skips any `deprecated.hard` agent as belt-and-suspenders. **Antigravity is untouched** — it keeps its `~/.gemini/antigravity-cli` config tree, the shared `updateGeminiSettings` writer (`permissions.ts`), the Google keychain plumbing (`find-generic-password -s gemini -a antigravity`), and the Gemini model names it runs (`pricing/table.ts`, `accounting/usage.ts`). Note: Gemini session-file parsing (`session/parse.ts`, `session/discover.ts`) is dropped with the agent, so historical Gemini transcripts are no longer read. Source: `cli/src/lib/types.ts`, `cli/src/lib/agent-spec/agents.ts`, `cli/src/lib/session/*`, `cli/src/lib/teams/parsers.ts`, `cli/src/lib/watchdog/*`, `cli/src/lib/rules/compile.ts`.

- **An interactive `agents run <harness> --device <windows box>` no longer dies with a PowerShell parse error.** From a TTY with no prompt, `run --device uranus` sent the POSIX remote command — `export AGENTS_REMOTE_INTERACTIVE=1; { cd "$HOME"/tools/cgraph || cd "$HOME"; } && agents run …` — to a Windows sshd whose default shell is PowerShell, which answered `At line:1 char:420` (the `||`) and closed the connection; only the headless `run --device` path rendered PowerShell. `runInteractiveOnHost` now picks the remote command by peer shell (`buildInteractiveRemoteCommand`): a Windows peer gets the same rendered `powershell -NoProfile` script every other `--device` site uses, with the actor env, the run-auto chain-hop guard and the interactive marker as `$env:` assignments and the argv carried through the argument-exact launcher; a POSIX peer is unchanged. Both Windows dispatch paths now share one `Set-Location` step (`windowsSetLocation`): a home-relative mirrored cwd (`~/tools/cgraph`) re-roots through `Join-Path $HOME` instead of a literal `'~/…'`, and its `$HOME` fallback is real — `Set-Location` on a missing directory is a non-terminating error, so the previous `try { … } catch { … }` never caught it. `--copy-creds` on an interactive Windows dispatch fails loud instead of being dropped. Source: `cli/src/lib/hosts/dispatch.ts`, `cli/src/lib/hosts/dispatch.test.ts`, `cli/src/lib/hosts/remote-cmd.ts`, `cli/src/lib/hosts/remote-cmd.test.ts`, `cli/docs/hosts.md`.
...
- **Account slots now follow the version home on every sync (PHNX-3940).** `agents accounts add` projected a slot's settings, skills, commands and hooks once and never again, so `agents sync` (and `sync <agent>@all`, the umbrella verb, SessionStart autosync) updated the version home and left every account on the resources it was cloned with. The post-reconcile repair now re-projects each slot of the reconciled harness from its default version home and prunes name-keyed artifacts the source no longer carries; the sync output lists each slot as `slot  account <name> ← <version>` and `--json` carries `repair.slotProjection`. Credentials are still never copied.

- **A worker's durable api-key slot now carries the pushed key into the launch, so `agents run cursor#gmail` (and codex/grok/opencode/droid api-key accounts) authenticate on a worker (PHNX-3940).** `credential-management.md` promised that an api-key harness gets a durable slot with no file and the key injected at spawn, but the native-account spawn branch in `commands/exec.ts` produced no env at all (its `SpawnAccount` contract says a native login needs none), so after the key had reached the box and the slot was provisioned, Cursor still answered `Authentication required. Please run 'agent login' first, or set CURSOR_API_KEY environment variable.` `durableSlotEnv` (`lib/exec-account-home.ts`) returns `{ <harness env>: <reserved key> }` exactly when the resolved slot is `durable`, the harness has an api-key worker kind, the row carries an api-key `workerCredential` and the key is readable on the box; a native login in the slot (headed device), a claude durable slot (its setup-token is a `.oauth_token` file) and a legacy home inject nothing. Source: `cli/src/lib/exec-account-home.ts`, `cli/src/lib/exec-account-home.test.ts`, `cli/src/commands/exec.ts`.

- **`agents sessions` read queries work again on a box without the standalone `sessions` CLI (PHNX-4012 follow-up).** The read fast-path in `cli/src/index.ts` exec'd `@phnx-labs/sessions-cli` for search/list/id lookups and, when that binary was missing, refused with `The standalone \`sessions\` CLI is not installed` and exit 1. That package is not published yet, so every fleet worker and every CI runner is such a box: main's attestation suite went red on `sessions.cli-list` and `sessions.fleet-json` (the PR's own scoped test job never ran them), which blocked every release. A missing standalone now falls through to the in-process engine with a one-line stderr hint (`AGENTS_SESSIONS_FASTPATH_HINT=0` silences it); a box that has `sessions` installed still takes the fast path. Source: `cli/src/index.ts`, `cli/src/index.sessions-fastpath.test.ts`.

- **A pushed bundle now lands in the root the receiving agents-cli reads, so a worker actually provisions the slot; and a Cursor slot launch on a worker no longer answers "not installed" (PHNX-3940).** Two seams closed after 1.22.90 delivered the `__cursor__` push. (1) agents-cli runs its local `secrets` under `SECRETS_HOME=~/.agents` (MIG-1) and the worker daemon reads pushed bundles from that root, but the remote half of the push ran the receiving `secrets` under its own default `~/.secrets`: zion logged `auth-sync: pushed __cursor__ (1 key(s)) to yosemite-m0` while yosemite-m0 kept answering `no readable durable key on this box yet` — the bundle sat in `~/.secrets/.cache/secrets`, keyed by a different machine-local passphrase than `~/.agents`. The one client wrapper every push goes through (`pushBundleToHost` / `pushBundleToHostAsync` in `secrets-client.ts`) now names the remote root (`withRemoteStateRoot`, `REMOTE_USER_AGENTS_DIR = '~/.agents'`), which secrets-cli 0.1.2 applies to the import, the read-back verify and literal restoration (`PushBundleOptions.remoteSecretsHome`). (2) With the key in place, `agents run cursor#gmail` on the worker still failed: the account slot launch hands the direct alias `HOME=<slot dir>` (real home in `AGENTS_REAL_HOME`), and the alias anchored its binary on `$HOME`, so `cursor-agent@main` reported `cursor@main not installed`; even under a plain HOME, cursor's own HOME swap ran before the launch lease, and `agents __launch-lease` (state root `$HOME/.agents`) failed with `No installation directory for cursor@main`. The versioned alias (schema 20 — an on-disk alias is regenerated by the daemon's self-heal tick within 6 h, or at once by `agents doctor --fix` / `agents view`; `agents upgrade` alone does not rewrite it) now anchors every agents-owned path on `AGENTS_REAL_HOME`, takes the launch lease under the real home before any harness HOME swap, and cursor swaps HOME only when the spawner has not already chosen one. The bare dispatcher shim (`cursor-agent` off PATH, `generateShimScript`) is unchanged: an account slot launch always resolves a version and runs the versioned alias, so it is not on this path; a hand-set `HOME` against the bare shim remains a separate gap. Test pin `STANDALONE_SECRETS_VERSION` → 0.1.2. Source: `cli/src/lib/secrets-client.ts`, `cli/src/lib/secrets-client.test.ts`, `cli/src/lib/secrets-types.ts`, `cli/src/lib/installations/shims.ts`, `cli/src/lib/installations/shims.test.ts`, `cli/tests/secrets-standalone.ts`.
...
- **`agents sessions --cloud <id>` (and `agents cloud view`) can now open OpenCode cloud transcripts (PHNX-3845).** Two gaps blocked them. (1) The cloud fetch (`ensureCloudSessionCached`) rejected any `X-Session-Format` outside `claude`/`codex`/`rush`, and discovery's `agentToFormat` dropped `opencode` rows — so an opencode run 404'd on fetch and never appeared in the list. Both now accept `opencode`, caching it as `session.opencode.jsonl`. (2) OpenCode is the one harness with no on-disk JSONL: locally the parser reads a `opencode.db#<session>` SQLite composite, but the cloud stores the factory's NORMALIZED JSONL (`{role, part_type, part_data, time_created}` per line, plus a `todo` snapshot line) produced by `prix/factory` opencode-capture. Feeding that JSONL to the SQLite parser yields an empty transcript. A distinct `parseOpencodeCloud` reads the normalized JSONL — mirroring the server-side `parseOpencode` (prix/api) and the offline parser's event shape — and `TRANSCRIPT_PARSERS.opencode` routes to it when the file lives in the cloud session cache (`isCloudSessionPath`), leaving the local SQLite path untouched. The shell tool is named `bash` on real opencode (>=1.18.x), not `shell`, so `command` is mapped for both. Source: `cli/src/lib/session/cloud.ts`, `cli/src/lib/session/parse.ts`, `cli/src/lib/session/parse.opencode-cloud.test.ts`, `cli/src/lib/session/cloud.test.ts`.
