...
- **`agents sync` no longer creates `GEMINI.md` alias symlinks.** `compileRulesForProject` symlinked every agent's `instructionsFile` to `AGENTS.md`, including the hard-deprecated Gemini agent's `GEMINI.md` (Google retired the Gemini CLI; the spec is kept only so legacy sessions stay parseable). Every synced project and home dir accumulated dead `GEMINI.md → AGENTS.md` symlinks. The per-agent symlink loop now skips agents whose `deprecated.hard` is set, matching the convention already used in `capabilities.ts`, `MANAGED_AGENT_IDS` and `commands/modes.ts`. Live agents (Claude, Codex, Antigravity, Grok, …) are unchanged, and the removal/cleanup path still unlinks stray `GEMINI.md` from old installs. Source: `cli/src/lib/rules/compile.ts`, `cli/src/lib/rules/compile.test.ts`, `cli/tests/project-resources-pipeline.test.ts`.
