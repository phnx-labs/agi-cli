...
- **An `out of credits` mark no longer strands a Claude account for good.** A run refused with "You've hit your monthly spend limit" / "out of usage credits" writes a per-account `unavailable: out_of_credits` mark in `~/.agents/.cache/claude-usage.json`, and rotation (`balanced`, `available`) excludes a marked account. The mark had no clock and its only clear was a successful run on that account — a run the router refuses to launch, and one an explicit `--account` launch does not clear either. On one box three of four Claude accounts sat marked for up to 14 days while answering fine, and every `agents run claude` collapsed onto the one login that was actually refusing. The mark now records `notedAt` and expires after `OUT_OF_CREDITS_REPROBE_MS` (one hour), so the router tries the account again; a fresh refusal simply re-marks it. A mark written before it carried a clock reads as expired, so a stranded account is eligible on the next read with no cache surgery. `agents view` shows `out of credits` only while the mark is live. Source: `cli/src/lib/accounting/usage.ts`, `cli/src/lib/accounting/usage.test.ts`.
