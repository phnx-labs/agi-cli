...
- **A full sync no longer deletes Claude Code's own `skills/synced` bucket.** The skills orphan sweep removes every non-dot directory under a harness's `skills/` that is not in the current sync set. On Claude that included `synced`, which Claude Code writes itself, so the full sync that clears the flattened plugin-skill copies left by 1.22.117 (`animator` beside `create:animator`) would also have deleted it. The agent spec now names the directories a harness owns (`ownedSkillDirs`, `['synced']` for Claude), and the sweep never removes them. Everything else it swept before, it still sweeps. Source: `cli/src/lib/installations/versions.ts`, `cli/src/lib/agent-spec/agents.ts`, `cli/src/lib/types.ts`.
